...
export function ProjectCard({ project, index, compact = false, squarish = false, onClick }: ProjectCardProps) {
  ...
  return (
    <motion.article
      ...
      tabIndex={0}
      role="button"
      className={`group relative overflow-hidden border border-[color-mix(in_oklch,#c96e10_12%,transparent)] bg-[linear-gradient(180deg,color-mix(in_oklch,var(--bg-card)_94%,white)_0%,color-mix(in_oklch,var(--bg-card-hover)_88%,white)_100%)] text-left shadow-[0_8px_22px_rgba(15,23,42,0.045)] outline-none transition duration-300 hover:-translate-y-1 hover:border-[color-mix(in_oklch,#c96e10_42%,transparent)] hover:bg-[linear-gradient(180deg,color-mix(in_oklch,#c96e10_8%,var(--bg-card))_0%,color-mix(in_oklch,#c96e10_5%,var(--bg-card-hover))_100%)] hover:shadow-[0_16px_34px_color-mix(in_oklch,#c96e10_18%,transparent)] focus-visible:-translate-y-1 focus-visible:border-[color-mix(in_oklch,#c96e10_42%,transparent)] focus-visible:bg-[linear-gradient(180deg,color-mix(in_oklch,#c96e10_8%,var(--bg-card))_0%,color-mix(in_oklch,#c96e10_5%,var(--bg-card-hover))_100%)] focus-visible:shadow-[0_16px_34px_color-mix(in_oklch,#c96e10_18%,transparent)] ${
    ...
    >
      ...
      <div className={`relative flex h-full flex-col ${squarish ? 'gap-3' : 'gap-3.5'}`}>
        <div className="flex items-start justify-between gap-4">
          <h3 className="text-lg font-semibold tracking-tight text-[var(--text)] transition group-hover:text-[#c96e10] group-focus-visible:text-[#c96e10]">
            {project.title}
          </h3>
        </div>

        <p
          className={`overflow-hidden text-[13px] leading-6 text-[var(--text-dim)] [display:-webkit-box] [-webkit-box-orient:vertical] ${
        ...
        <div className="flex flex-wrap gap-2">
          {project.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-[color-mix(in_oklch,#c96e10_14%,transparent)] bg-[color-mix(in_oklch,#c96e10_8%,var(--bg-card))] px-2.5 py-0.5 text-[11px] font-medium text-[color-mix(in_oklch,#c96e10_78%,var(--text))] transition group-hover:border-[color-mix(in_oklch,#c96e10_24%,transparent)] group-hover:bg-[color-mix(in_oklch,#c96e10_12%,var(--bg-card))]"
            >
              {tag}
            </span>
          ))}
        ...
        <div className={`flex items-center justify-between gap-3 ${squarish ? 'mt-1 pt-0.5' : 'mt-auto pt-2'}`}>
          ...
          {project.githubLink ? (
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full border border-transparent px-2 py-1 text-[13px] font-medium text-[var(--text-dim)] transition hover:border-[var(--border-bright)] hover:bg-[var(--text)] hover:text-[var(--bg)]"
          ...
          {project.demoLink ? (
            <a
              ...
              rel="noreferrer"
              onClick={stopCardNavigation}
              className="inline-flex items-center gap-2 rounded-full border border-transparent px-2 py-1 text-[13px] font-medium text-[var(--text-dim)] transition hover:border-[color-mix(in_oklch,#c96e10_26%,transparent)] hover:bg-[color-mix(in_oklch,#c96e10_10%,var(--bg-card))] hover:text-[#c96e10]"
